...
const callable_vtbl<R, A...>& get_callable_vtbl() {
  ...
  static const callable_vtbl<R, A...> res = {[](small_buffer& buf) { reinterpret_cast<F&>(buf).~F(); },
...
template <typename T>
auto is_null(const T&) noexcept -> std::enable_if_t<!is_null_comparable<T>::value, bool> {
  return false;
}
